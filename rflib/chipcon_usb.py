...
class USBDongle(object):
    ...
    def reprHardwareConfig(self):
        ...
        def _ascii(b):
            return b.decode('ascii', errors='replace').rstrip('\x00').rstrip()

        hardware = self.getBuildInfo()
        output.append("Dongle:              %s" % _ascii(hardware.split(b' ')[0]))
        try:
            output.append("Firmware rev:        %s" % _ascii(hardware.split(b'r')[1]))
        except:
            output.append("Firmware rev:        Not found! Update needed!")
        try:
            compiler = self.getCompilerInfo()
            output.append("Compiler:            %s" % _ascii(compiler))
        except:
